...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "stop_progress" in self.list_data_dict[i]:
            data_dict["stop_progress"] = torch.as_tensor(
                self.list_data_dict[i]["stop_progress"], dtype=torch.float32
            ).reshape(-1)[0]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        has_stop_progress = any("stop_progress" in instance for instance in instances)
        if has_stop_progress:
            batch["stop_progress"] = torch.stack(
                [
                    instance.get("stop_progress", torch.tensor(float("nan"), dtype=torch.float32)).to(torch.float32)
                    for instance in instances
                ]
            )
...
class FlattenedDataCollatorForSupervisedDataset(DataCollatorForSupervisedDataset):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if any("stop_progress" in instance for instance in instances):
            raise NotImplementedError("FlattenedDataCollatorForSupervisedDataset does not support stop_progress")
