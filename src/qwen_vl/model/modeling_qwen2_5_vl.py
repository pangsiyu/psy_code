...
class Qwen2_5_VLCausalLMOutputWithPast(ModelOutput):
    ...
    stop_progress: Optional[torch.FloatTensor] = None
...
class Qwen2_5_VLForConditionalGenerationForJanusVLN(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.kv_cache_vggt = StartRecentKVCache(
                start_size=int(os.environ.get("VGGT_CACHE_START_SIZE", 8)),
                recent_size=int(os.environ.get("VGGT_CACHE_RECENT_SIZE", 16)),
                k_seq_dim=2,          
                v_seq_dim=2            
            )
        ...
        if getattr(config, "add_stop_progress_head", False):
            self.stop_progress_head = nn.Sequential(
                Qwen2RMSNorm(config.hidden_size, eps=getattr(config, "rms_norm_eps", 1e-6)),
                nn.Linear(config.hidden_size, getattr(config, "stop_head_hidden_dim", 1024)),
                nn.GELU(),
                nn.Linear(getattr(config, "stop_head_hidden_dim", 1024), 1),
                nn.Sigmoid(),
            )
        self.last_stop_progress = None
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, mode=None, **kwargs):
        ...
        if vggt_model_path:
            ...
            if os.path.isfile(vggt_model_path):
                vggt = VGGT()
                vggt_state = torch.load(vggt_model_path, map_location="cpu")
                if isinstance(vggt_state, dict):
                    for state_key in ("model", "state_dict", "module"):
                        if state_key in vggt_state and isinstance(vggt_state[state_key], dict):
                            vggt_state = vggt_state[state_key]
                            break
                missing_keys, unexpected_keys = vggt.load_state_dict(vggt_state, strict=False)
                if missing_keys:
                    print(f"VGGT missing keys when loading pth: {len(missing_keys)}")
                if unexpected_keys:
                    print(f"VGGT unexpected keys when loading pth: {len(unexpected_keys)}")
            else:
    ...
    def forward(
        ...
        stop_progress: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        stop_progress_pred = self._compute_stop_progress(
            hidden_states=hidden_states,
            labels=labels,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
        ...
        if stop_progress is not None and stop_progress_pred is not None:
            stop_progress_target = stop_progress.to(stop_progress_pred.device, dtype=stop_progress_pred.dtype).view(-1, 1)
            valid_stop_progress = torch.isfinite(stop_progress_target).view(-1)
            if valid_stop_progress.any():
                stop_loss = F.mse_loss(
                    stop_progress_pred[valid_stop_progress],
                    stop_progress_target[valid_stop_progress],
                )
                weighted_stop_loss = getattr(self.config, "stop_loss_weight", 10.0) * stop_loss
                loss = weighted_stop_loss if loss is None else loss + weighted_stop_loss


        if not return_dict:
            output = (logits,) + outputs[1:]
            if stop_progress_pred is not None:
                output = output + (stop_progress_pred,)
        ...
        return Qwen2_5_VLCausalLMOutputWithPast(
            ...
            stop_progress=stop_progress_pred,
        )

    def _compute_stop_progress(
        self,
        hidden_states: torch.Tensor,
        ...
        if not getattr(self.config, "add_stop_progress_head", False):
            self.last_stop_progress = None
            return None
        ...
        if labels is not None:
            ...
            decision_idx = (first_label_idx - 1).clamp(min=0)
            if attention_mask is not None:
                last_valid_idx = attention_mask.to(hidden_states.device).long().sum(dim=1).sub(1).clamp(min=0)
            else:
                last_valid_idx = torch.full((batch_size,), seq_len - 1, device=hidden_states.device, dtype=torch.long)
            decision_idx = torch.where(has_label.to(hidden_states.device), decision_idx.to(hidden_states.device), last_valid_idx)
        else:
            is_generation_prefill = self._is_generation_prefill(past_key_values, cache_position)
            if not is_generation_prefill and self.last_stop_progress is not None:
                return self.last_stop_progress
            if not is_generation_prefill and seq_len <= 1:
                return self.last_stop_progress
            if attention_mask is not None:
                decision_idx = (
                    ...
                    .sum(dim=1)
                    .sub(1)
                    .clamp(min=0, max=seq_len - 1)
                )
            else:
                decision_idx = torch.full((batch_size,), seq_len - 1, device=hidden_states.device, dtype=torch.long)
        ...
        stop_progress_pred = self.stop_progress_head(decision_hidden)
        stop_progress_pred = torch.nan_to_num(stop_progress_pred, nan=0.0, posinf=1.0, neginf=0.0)
        self.last_stop_progress = stop_progress_pred.detach()
        return stop_progress_pred

    @staticmethod
    def _is_generation_prefill(past_key_values, cache_position) -> bool:
        if cache_position is not None:
            return int(cache_position[0].item()) == 0
        if past_key_values is None:
            return True
