...
class Qwen2_5_VLForConditionalGenerationForJanusVLN(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.topo_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(1024, 5)  # 分类：走廊, 房间, 十字路口, 死胡同, 门道
        )
        # 控制拓扑 Loss 在总 Loss 中的占比
        self.gamma = 0.5
        
        # 2. 【新增】单目度量校准头 (Distance Regression Head)
        self.dist_head = nn.Sequential(
            nn.Linear(config.hidden_size, 512),
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(512, 1)  # 回归预测剩余距离 (米)
        )
        self.beta = 1.0  # 距离 Loss 在总 Loss 中的占比
        
        # 3. 【新增】跨模态拓扑注意力融合 (Cross-Attention Alignment)
        self.instruction_spatial_attn = nn.MultiheadAttention(
            embed_dim=config.hidden_size, 
            num_heads=8, 
            batch_first=True,
            dropout=0.1
        )
        self.cross_attn_norm = Qwen2RMSNorm(config.hidden_size, eps=1e-6)
        # ============================================================
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, mode=None, **kwargs):
        ...
        model.mode = mode
        if vggt_model_path:
            print(f"Loading VGGT from local PyTorch weights: {vggt_model_path}")
            # 1. 先实例化一个空白的 VGGT 模型
            vggt = VGGT()
            
            # 2. 使用原生 torch.load 读取 .pth 文件
            state_dict = torch.load(vggt_model_path, map_location="cpu")
            # 处理可能的 state_dict 嵌套 (有些权重保存在 'model' 或 'state_dict' 键下)
            if "model" in state_dict:
                state_dict = state_dict["model"]
            elif "state_dict" in state_dict:
                state_dict = state_dict["state_dict"]
                
            # 3. 将权重严格加载进空白模型 (允许缺失一些 head 的权重)
    ...
    def forward(
        ...
        distance_labels: Optional[torch.FloatTensor] = None, # 真实距离标签
        # ==================================
        **kwargs,
        ...
        >>> url = "[https://www.ilankelman.org/stopsigns/australia.jpg](https://www.ilankelman.org/stopsigns/australia.jpg)"
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # ================== 创新点：跨模态注意力融合 ==================
                # 原逻辑：image_embeds = image_embeds + self.lam * image_embeds_3d
                # 注意力逻辑：以当前2D视觉(含指令上下文)检索3D空间记忆
                q_2d = image_embeds.view(batch_size, -1, image_embeds.shape[-1])
                kv_3d = image_embeds_3d.view(batch_size, -1, image_embeds_3d.shape[-1])
                
                attn_output, _ = self.instruction_spatial_attn(query=q_2d, key=kv_3d, value=kv_3d)
                # 残差连接 + Norm
                fused_embeds = self.cross_attn_norm(q_2d + self.lam * attn_output)
                image_embeds = fused_embeds.view(-1, image_embeds.shape[-1])
                # ==============================================================
        ...
        # ================== 创新点：半监督拓扑 Loss 计算 ==================
        if unlabeled_images is not None and topo_labels is not None:
            try:
                # unlabeled_images 从 data_qwen 传进来时是 [B, L_patches, Channel]
                B, L, C = unlabeled_images.shape
                flattened_unlabeled = unlabeled_images.view(-1, C).to(self.visual.dtype)
                
                # 自动推断 grid_thw (Qwen2-VL 默认 Patch 处理后，长宽维度近似相等)
                # 即使没有 3D VGGt 数据支撑，纯靠自我视觉编码器的全局特征依然能对齐拓扑表征
                grid_size = int(math.sqrt(L))
                unlabeled_grid_thw = torch.tensor(
                    [[1, grid_size, grid_size]], 
                    dtype=torch.long, 
                    device=unlabeled_images.device
                ).repeat(B, 1)
                
                # 1. 提取视觉特征 (复用原生 Qwen 视觉感知器，借此打磨其内在空间结构意识)
                unlabeled_features = self.visual(flattened_unlabeled, grid_thw=unlabeled_grid_thw)
                
                # 2. 将特征重新 Reshape 回 [B, N_patches, Hidden_size]
                hidden_dim = unlabeled_features.shape[-1]
                unlabeled_features = unlabeled_features.view(B, -1, hidden_dim)
                
                # 3. 对画面全局进行平均池化 (Global Average Pooling)
                pooled_features = unlabeled_features.mean(dim=1)
                
                # 4. 前向传播：预测当前视角的空间拓扑，并计算交叉熵 Loss
                topo_logits = self.topo_head(pooled_features)
                loss_fct_topo = CrossEntropyLoss()
                topo_loss_val = loss_fct_topo(topo_logits, topo_labels.view(-1))
                
                # 5. 联合 Loss 反向传播
                if loss is None:
                    loss = self.gamma * topo_loss_val
                else:
                    loss += self.gamma * topo_loss_val
                    
            except Exception as e:
                # 兜底机制：一旦因网络截断或假数据张量越界报错，直接放行主监督 Loss 继续训练
                print(f"\n[Warning] Semi-Supervised Topo Loss Skipping due to exception: {e}")
        # ================================================================

        # ================== 创新点：单目度量回归 Loss ==================
        if distance_labels is not None:
            try:
                # 取 LLM 最后一个输出 Token 的状态作为当前决策状态预测距离
                decision_state = hidden_states[:, -1, :] 
                dist_pred = self.dist_head(decision_state).squeeze(-1)
                loss_fct_dist = nn.MSELoss()
                dist_loss_val = loss_fct_dist(dist_pred, distance_labels.to(dist_pred.device))
                loss = (self.beta * dist_loss_val) if loss is None else loss + (self.beta * dist_loss_val)
            except Exception as e:
                print(f"\n[Warning] Distance Loss Skipping: {e}")
        # ===============================================================
