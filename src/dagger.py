...
class DAggerCollector:
    ...
    def generate(
        ...
        rgb_list,  time_ids = [], []
        if evaluator is not None and hasattr(evaluator.model, 'model'):
                ...
                if len(action_seq) == 0:
                    if left_expert_actions_num > 0:
                        ...
                        if from_expert:
                            ...
                            # 核心修复：调用call_model时加try-except，避免崩溃
                            try:
                                action = evaluator.model.call_model(images, instructions, int(step_id))[0]
                                if action in evaluator.actions2idx:
                                    action = evaluator.actions2idx[action][0]
                                else:
                                    action = 0
                            except Exception as e:
                                # 捕获所有异常，强制使用默认动作，避免终止
                                print(f"[紧急修复] 帧{step_id}推理出错：{e}，使用默认动作MOVE_FORWARD")
                                action = 0
                            action_seq = [action]
...
class JanusVLN_Inference:
    def __init__(self, pretrained, device="cuda"):
        # 禁用FlashAttention，改用eager
        config = AutoConfig.from_pretrained(pretrained)
        config.attn_implementation = "eager"
        
        # 显存优化：bfloat16→float16
    ...
    def call_model(
        ...
        try:
            # ========== 终极修复：强制赋值+调试打印 ==========
            # 1. 强制覆盖patch_size/merge_size，无视原有值
            self.processor.image_processor.patch_size = 14  # 固定整数，永不None
            self.processor.image_processor.merge_size = 2   # 固定整数，永不None
            # 2. 读取并强制转为整数
            patch_size = int(self.processor.image_processor.patch_size)
            merge_size = int(self.processor.image_processor.merge_size)
            # 3. 调试打印（确认变量值，可后续删除）
            print(f"[调试] patch_size={patch_size}, merge_size={merge_size}, type={type(patch_size)}")
            ...
            for message in messages:
                ...
                for i, ele in enumerate(vision_info):
                    ...
                    patch_size = patch_size if patch_size > 0 else 14
                    merge_size = merge_size if merge_size > 0 else 2

                    # 安全计算：避免所有None/0的情况
                    try:
                        width_div = width // patch_size
                        width_mod = width_div % merge_size
                        if width_mod > 0:
                            width = width - width_mod * patch_size
                    except:
                        width = 0  # 出错时强制设为0，避免崩溃
                    
                    try:
                        height_div = height // patch_size
                        height_mod = height_div % merge_size
                        if height_mod > 0:
                            height = height - height_mod * patch_size
                    except:
                        height = 0  # 出错时强制设为0，避免崩溃

                    # 边界校验
                    height = max(0, height)
                    width = max(0, width)
                    image = image[:, :height, :width] if (height > 0 and width > 0) else image
            ...
            inputs["images_vggt"] = [feat.to(device) for feat in images_vggt] if images_vggt else []
            inputs = inputs.to(device)
        
            gen_kwargs = gen_kwargs or {}
            gen_kwargs["max_new_tokens"] = gen_kwargs.get("max_new_tokens", 24)
            gen_kwargs["temperature"] = gen_kwargs.get("temperature", 0)
            gen_kwargs["top_p"] = gen_kwargs.get("top_p", None)
            gen_kwargs["num_beams"] = gen_kwargs.get("num_beams", 1)
            ...
            return answers if answers else ["MOVE_FORWARD"]
        
        except Exception as e:
            # 捕获所有异常，打印详细信息，返回默认动作
            print(f"[call_model异常] 详细错误：{traceback.format_exc()}")
            return ["MOVE_FORWARD"]
